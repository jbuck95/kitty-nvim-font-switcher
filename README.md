...
```bash
git clone https://github.com/jbuck95/kitty-nvim-font-switcher.git
cd kitty-nvim-font-switcher
./install.sh
```

The script copies font files and Kitty configs. After that:

1. Add `listen_on unix:/tmp/kitty` to your `kitty.conf`
2. Add to your Neovim config:
   ...
   vim.api.nvim_create_autocmd({ "FileType", "BufWinEnter" }, {
     pattern = "*", group = group, callback = switch_kitty_font,
   })
   ...
   ```
   Or require the standalone file:
   ```lua
   require('config.kitty-font-switcher')
   ```
3. Install `ttf-jetbrains-mono-nerd` (Arch) or equivalent
